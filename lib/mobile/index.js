...
var get_number = function(numbers,number,lines) {
...
var get_data = function(filepath) {
    ...
    blocks.forEach(function(block) {
        ...
        if (lines[0] === 'type') {
            data.type = get_type(lines.slice(1));
        } else if (lines[0] === 'area') {
            data.area = get_area(lines.slice(1));
        } else if (lines[0].substr(0, 7) === 'number-') {
            data.number = get_number(data.number,lines[0].substr(7), lines.slice(1));
        }
    });
    return data;
};
...
var query = function(number) {
    ...
    var data = get_data(filepath.mobile.datapath);
...
var query_country = function(number) {
